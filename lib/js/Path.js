var Path = _class("Path", {

  mixins: [Base],

  interfaces: [Shape],

  props: {
    id: '',
    _position: {x: 0, y: 0},
  },

  methods: {
    init: function (id, points)
    {
      this.id = id;
      this.impl = new IMPL.Path(id);
      this.points(points);
    },

    /**
     *
     * M   moveto                           (x y)+
     * Z   closepath                        (none)
     * L   lineto                           (x y)+
     * H   horizontal lineto                x+
     * V   vertical lineto                  y+
     * C   curveto                          (x1 y1 x2 y2 x y)+
     * S   smooth curveto                   (x2 y2 x y)+
     * Q   quadratic Bezier curveto         (x1 y1 x y)+
     * T   smooth quadratic Bezier curveto  (x y)+
     * A   elliptical arc                   (rx ry x-axis-rotation large-arc-flag sweep-flag x y)+
     * R   Catmull-Rom curveto*x1 y1        (x y)+
     *
     **/
    points: function(points, copyless)
    {

      if (points !== undefined) {

        if (typeof points === 'string') {
          this._points = this._convertPointString(points);
          this.impl.points(this._points);

        } else if (points instanceof Array) {
          this._points = (copyless) ? points : _clone(points);
          this.impl.points(this._points);

        } else {
          _error('Argument Error', 'points() expects an Array or a String as an argument.');

        }

        this._updateState();
      }

      return this._points;
    },

    pointAt: function(i, point, copyless)
    {
      if (i < this._points.length) {
        if (point !== undefined) {
          if (typeof point === 'string') {
            point = this._convertPointString(point)[0];
            copyless = true;
          }
          if (point instanceof Array) {
            this._points[i] = (copyless) ? point : _clone(point);
            this.points(this._points, true);

          } else {
            _error();

          }
        }
      } else {
        _error();
      }

      return this._points[i];
    },

    position: function(d)
    {
      if (d) {
        var lx = this._position.x;
        var ly = this._position.y;
        var xm = d.x - lx;
        var ym = d.y - ly;
        var points = this._points;

        var mat = (new Lib.Matrix()).translate(xm, ym);
        this._applyMatrixToPoints(mat);

        this.points(this._points, true);
      }

      return { x: this._position.x, y: this._position.y };
    },

    size: function(d)
    {
      if (d) {
        var lw = this._size.width;
        var lh = this._size.height;
        var wm = d.width / lw;
        var hm = d.height / lh;
        var pos = this._position;

        var mat = (new Lib.Matrix()).scale(wm, hm, pos.x, pos.y);
        this._applyMatrixToPoints(mat);

        this.points(this._points, true);
      }
      return {width: this._size.width, height: this._size.height};
    },

    displayPosition: function()
    {
    },

    displaySize: function()
    {
    },

    gravityPosition: function()
    {
    },

    hitTest: function(d)
    {
    },

    /* private methods */

    _getIdentifierArglen: function(x) {
      switch (x) {
      case 77: // M
      case 76: // L
      case 84: // T
      case 82: // R
        return 2;
      case 67: // C
        return 6;
      case 90: // Z
        return 0;
      case 72: // H
      case 86: // V
        return 1;
      case 83: // S
      case 81: // Q
        return 4;
      case 65: // A
        return 7;
      }
      _error('Invalid pointString identifier Error',
             'Expected identifiers are M,L,H,V,C,Z,T,R,S,Q,A. but you gave "' + String.fromCharCode(x) + '".');
    },

    _convertPointString: function(points) {

      var x, atom, arglen, arglen_now, set, last_idt, rt = [], arr = points.split(SEPARATOR);

      atom = arr[0];
      if (!( atom.length === 1 && (x = atom.charCodeAt(0)) && 64 < x && x < 91 )) {
        _error('Invalid pointString format Error', 'First item must be an identifier M,L,H,V,C,Z,T,R,S,Q,A.');
      }
      set = [atom];
      last_idt = atom;
      arglen = this._getIdentifierArglen(x);
      arglen_now = 0;

      for (var i=1, l=arr.length; i<l; i++) {

        if ((atom = arr[i]) === '') continue;

        x = atom.charCodeAt(0);

        if ( atom.length === 1 && 64 < x && x < 91 ) {

          if (arglen === arglen_now) {
            rt.push(set);
            set = [atom];
            last_idt = atom;
            arglen = this._getIdentifierArglen(x);

          } else {
            _error('Invalid pointString format Error', 'The arguments length of the identifier \''+ last_idt +'\' is not accurate. ' +
                   'Expected ' + arglen + ' or multiple of it,  but you gave ' + arglen_now + ' unnecessary arguments.');

          }

        } else if ((47 < x && x < 58) || x === 43 || x === 45 || x === 46) {
          n =  +(parseFloat(atom).toFixed(FLOAT_ACCURACY));
          if (arglen === arglen_now) {
            rt.push(set);
            set = [last_idt, n];

          } else {
            set.push(n);

          }
        } else {
          _error('Invalid pointString format Error', 'Point string is not allowed except for identifier-string M,L,H,V,C,Z,T,R,S,Q,A or a Number.' );

        }

        arglen_now = set.length - 1;

      }

      if (arglen !== arglen_now)
        _error('Invalid pointString format Error', 'The arguments length of the identifier \''+ last_idt +'\' is not accurate.' +
               'Expected ' + arglen + ' or multiple of it,  but you gave ' + arglen_now + ' unnecessary arguments.');

      rt.push(set);

      return rt;

    },

    /* private */
    _updateState: function()
    {
      var x, y, pos = {x_min: Infinity, y_min: Infinity,
                       x_max: -Infinity, y_max: -Infinity};

      var points = this._points;

      for (var i=0, l=points.length, ll = l-1; i<l; i++) {
        var item = points[i];
        var idt = (item[0]).charCodeAt(0);

        switch (idt) {

        case 77: // M
          if (i < ll) {
            next_idt = points[i+1][0];
            if (next_idt !== 'M') {
              x = item[1]; y = item[2];
            }
          }
          break;

        case 76: // L
        case 84: // T // TODO: consider curving line.
        case 82: // R // TODO: consider curving line.
          x = item[1]; y = item[2];
          break;

        case 67: // C // TODO: consider curving line.
          x = item[5]; y = item[6];
          break;

        case 90: // Z
          break;

        case 72: // H
          x = item[0];
          break;

        case 86: // V
          y = item[0];
          break;

        case 83: // S // TODO: consider curving line.
        case 81: // Q // TODO: consider curving line.
          x = item[3]; y = item[4];
          break;

        case 65: // A // TODO: consider curving line.
          x = item[6]; y = item[7];
          break;
        }

        if (pos.x_min > x) pos.x_min = x;
        if (pos.x_max < x) pos.x_max = x;

        if (pos.y_min > y) pos.y_min = y;
        if (pos.y_max < y) pos.y_max = y;

      }

      this._position.x = pos.x_min;
      this._position.y = pos.y_min;

      this._size.width  = pos.x_max - pos.x_min;
      this._size.height = pos.y_max - pos.y_min;

    },

    _applyMatrixToPoints: function(matrix) {

      var idt, points, p, x, y, last_x, last_y;
      last_x = last_y = 0;
      points = this._points;

      loop:
      for (var i=0, l=points.length; i<l; i++) {

        p = points[i];
        idt = (p[0]).charCodeAt(0);

        switch (idt) {

        case 77: // M
        case 76: // L
        case 84: // T
        case 82: // R
          x = [1]; y = [2];
          last_x = p[1]; last_y = p[2];
          break;

        case 67: // C
          x = [1, 3, 5];
          y = [2, 4, 6];
          last_x = p[5]; last_y = p[6];
          break;

        case 90: // Z
          continue loop;

        case 72: // H
          x = [1]; y = [0];
          last_x = p[1];
          break;

        case 86: // V
          x = [0]; y = [1];
          last_y = p[1];
          break;

        case 83: // S
        case 81: // Q
          x = [1, 3];
          y = [2, 4];
          last_x = p[3]; last_y = p[4];
          break;

        case 65: // A
          x = [1, 6];
          y = [2, 7];
          last_x = p[6]; last_y = p[7];
          break;
        }

        for (var j=0, m=x.length; j<m; j++) {
          var x_idx = x[j]; var y_idx = y[j];

          p[x_idx] = matrix.x((x_idx) ? p[x_idx] : last_x,
                              (y_idx) ? p[y_idx] : last_y);

          p[y_idx] = matrix.y((x_idx) ? p[x_idx] : last_x,
                              (y_idx) ? p[y_idx] : last_y);

        }
      }
    }
  }
});
