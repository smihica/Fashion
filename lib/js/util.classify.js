var _class = (function() {
  ...
  return function _class(name, definition) {
    ...
    for(j=0, l=includes.length; j<l; j++) {
      mixin(__class__, includes[j]);
    }
    ...
    for (j=0, l=impls.length; j<l; j++) {
      check_interface(__class__, impls[j]);
    }
